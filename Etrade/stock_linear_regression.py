...
# testing
    ...
    for i in range(TESTING_SIZE -1):
        ...
        updown_pred = res[0][0]
        if (updown_pred > 0.33) and (0 == owned):
            owned = money
            buyed_money = money
            money -= money * 0.0033 # charge and tax


        elif (updown_pred < -0.0) and ( 0 != owned) and ( money > buyed_money):
            owned = 0
            money -= money * 0.0033 # charge and tax
        ...
        int_depot.append(depot_only)
        int_pred.append(money)
        # _, c = sess.run([optimizer, cost], feed_dict={X: x, Y: y})
 ...
 #   print(predicted)
    ...
    plt.plot(day, int_pred, label="Exp money")
    plt.plot(day, int_depot, label = "deposit money")
    plt.legend()
    plt.show()
